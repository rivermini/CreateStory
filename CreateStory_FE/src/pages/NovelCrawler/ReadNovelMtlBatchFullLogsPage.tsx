...
import { getReadNovelMtlBatchLogs, type ReadNovelMtlBatchLogsResponse, type ReadNovelMtlBatchPhase } from '../../api';
import { Icon, appIcons } from '../../components/Shared/Icon';
import type { ThemeMode } from '../../types/theme';
import { getReadNovelMtlLogTone, readnovelmtlLogToneClass, splitReadNovelMtlLogLine, type ReadNovelMtlLogTone } from './readnovelmtlLogUtils';

interface ReadNovelMtlBatchFullLogsPageProps {
  readonly themeMode: ThemeMode;
}

const TONE_LABELS: Array<{ tone: ReadNovelMtlLogTone | 'all'; label: string }> = [
  { tone: 'all', label: 'All' },
...
function phaseLabel(phase: ReadNovelMtlBatchPhase | undefined): string {
...
export function ReadNovelMtlBatchFullLogsPage({ themeMode }: ReadNovelMtlBatchFullLogsPageProps) {
  const { batchId = '' } = useParams();
  const isDark = themeMode === 'dark';
  const [payload, setPayload] = useState<ReadNovelMtlBatchLogsResponse | null>(null);
  const [error, setError] = useState('');
  const [loading, setLoading] = useState(false);
  const [query, setQuery] = useState('');
  const [toneFilter, setToneFilter] = useState<ReadNovelMtlLogTone | 'all'>('all');
  ...
  const fetchLogs = useCallback(async () => {
    ...
    setError('');
    try {
      const response = await getReadNovelMtlBatchLogs(batchId);
      setPayload(response);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load ReadNovelMtl full logs.');
    } finally {
      setLoading(false);
    }
  }, [batchId]);
  ...
  const enrichedLines = useMemo(() => {
    return (payload?.log_lines || []).map((line, index) => ({
      index,
      line,
      tone: getReadNovelMtlLogTone(line),
      parts: splitReadNovelMtlLogLine(line),
    }));
  }, [payload]);

  const toneCounts = useMemo(() => {
    const counts: Record<ReadNovelMtlLogTone | 'all', number> = {
          ...
          <section className="rounded-lg border px-4 py-4 sm:px-5" style={{ background: panelBg, borderColor: panelBorder }}>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Link to="/readnovelmtl-batch" className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm font-semibold" style={{ borderColor: panelBorder, background: muted, color: text }}>
            ...
            <div className="max-h-[calc(100vh-330px)] min-h-[420px] overflow-auto p-2 font-mono text-xs leading-5" style={{ background: muted }}>
              {loading && !payload ? (
                ...
                visibleLines.map((item) => (
                  <div key={`${item.index}-${item.line}`} className={`mb-1 rounded border-l-2 px-2 py-1 last:mb-0 ${readnovelmtlLogToneClass(item.tone)}`}>
