export type ReadNovelMtlLogTone =
  | 'error'
  | 'warning'
  | 'success'
  | 'progress'
...
export function getReadNovelMtlLogTone(line: string): ReadNovelMtlLogTone {
  ...
  if (/\b(system catalog|catalog updated|started readnovelmtl)\b/.test(lower)) return 'system';
  return 'neutral';
}

export function splitReadNovelMtlLogLine(line: string): { time: string; message: string } {
  const match = /^(\d{2}:\d{2}:\d{2})\s+(.*)$/.exec(line);
  if (!match) return { time: '', message: line };
  return { time: match[1], message: match[2] };
}

export function readnovelmtlLogToneClass(tone: ReadNovelMtlLogTone): string {
  switch (tone) {
    case 'error':
