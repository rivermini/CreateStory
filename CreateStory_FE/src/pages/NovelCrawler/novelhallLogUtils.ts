...
export function getNovelHallLogTone(line: string): NovelHallLogTone {
  ...
  if (/\b(system catalog|catalog updated|started novelhall)\b/.test(lower)) return 'system';
  return 'neutral';
}

export function splitNovelHallLogLine(line: string): { time: string; message: string } {
  const match = /^(\d{2}:\d{2}:\d{2})\s+(.*)$/.exec(line);
  if (!match) return { time: '', message: line };
  return { time: match[1], message: match[2] };
}

export function novelhallLogToneClass(tone: NovelHallLogTone): string {
  switch (tone) {
    case 'error':
