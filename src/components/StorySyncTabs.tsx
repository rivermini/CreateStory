...
function ValidationErrorBadge({ error, isDark }: { error: string; isDark: boolean }) {
  ...
  return (
    <span className={`flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold rounded-lg border ${
      isFormat
        ? isDark
...
function UploadableTab({
  ...
  return (
    <div className="p-4 flex flex-col h-full">
      <div className="flex items-center justify-between flex-shrink-0 flex-wrap gap-3">
        <div className="flex items-center gap-2">
          <button
            onClick={onCheck}
            disabled={loading}
            className={`px-4 py-2 text-sm font-semibold rounded-xl transition-all duration-200 flex items-center gap-2 shadow-lg ${
                loading
                  ? isDark
        ...
        <div className="flex items-center gap-2 flex-shrink-0 my-4 px-1">
          <div className="relative flex-1">
            ...
            <input
              ...
              className={`w-full pl-9 pr-8 py-2 rounded-xl border text-sm
                ${isDark
                  ? 'bg-slate-900/60 border-slate-800 text-slate-200 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500'
      ...
      {error && (
        <div className={`p-3 rounded-xl text-sm mb-3 ${isDark ? 'bg-red-900/20 border border-red-800/30 text-red-400' : 'bg-red-50 border border-red-200 text-red-600'}`}>
          {error}
        </div>
      )}
          ...
          {filteredInvalid.length > 0 && (
            <div>
              ...
              {filteredInvalid.map(folder => (
                <div key={folder.id} className={`flex items-center gap-3 p-3 rounded-xl border mt-2 ${isDark ? 'bg-red-950/20 border-red-800/30' : 'bg-red-50 border-red-200'}`}>
                  <span className={`inline-flex items-center px-2 py-0.5 text-xs rounded-lg border ${statusColor(folder.prefix)}`}>
                    {folder.prefix}
                  </span>
                  ...
                  <span className={`px-2 py-1 text-xs rounded-lg whitespace-nowrap ${isDark ? 'text-red-400 bg-red-900/40 border border-red-800/40' : 'text-red-600 bg-red-100 border border-red-200'}`}>
                    Cannot Upload
                  </span>
                </div>
              ))}
          ...
          {validCount > 0 && (
            <div>
              ...
              {filteredUploadable.map(folder => {
                return (
                  <div key={folder.id} className={`flex items-center gap-3 p-3 rounded-xl border mt-2 ${isDark ? 'bg-slate-900/40 border-slate-800/60' : 'bg-gray-50 border-gray-200'}`}>
                    <span className={`inline-flex items-center px-2 py-0.5 text-xs rounded-lg border ${statusColor(folder.prefix)}`}>
                      {folder.prefix}
                    </span>
                    <div className="flex-1 min-w-0">
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-semibold rounded-xl transition-all duration-200 flex items-center gap-1.5 whitespace-nowrap shadow-lg ${
          ...
          {filteredAlready.length > 0 && (
            <div>
              ...
              {filteredAlready.map(folder => (
                <div key={folder.id} className={`flex items-center gap-3 p-3 rounded-xl mt-2 ${isDark ? 'bg-slate-900/20 border border-slate-800/40 opacity-60' : 'bg-gray-100 border border-gray-200 opacity-60'}`}>
                  <span className={`inline-flex items-center px-2 py-0.5 text-xs rounded-lg border ${statusColor(folder.prefix)}`}>
                    {folder.prefix}
                  </span>
                  <div className="flex-1 min-w-0">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-lg ${isDark ? 'text-slate-500 bg-slate-800/40' : 'text-gray-500 bg-gray-200'}`}>Already uploaded</span>
                </div>
              ))}
            </div>
          )}
          ...
          {filteredUpdatable.length > 0 && (
            <div>
              ...
              {filteredUpdatable.map((entry: UpdatableStoryEntry) => {
                ...
                return (
                  <div key={entry.server_story.id} className={`flex items-center gap-3 p-3 rounded-xl mt-2 ${isDark ? 'bg-slate-900/40 border border-slate-800/60' : 'bg-gray-50 border border-gray-200'}`}>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        {delta > 0 && (
                          <span className={`px-2 py-0.5 text-[10px] font-bold rounded-lg border ${isDark ? 'bg-amber-900/40 text-amber-300 border-amber-800/40' : 'bg-amber-100 text-amber-700 border-amber-200'}`}>
          ...
          {filteredInvalid.length > 0 && (
            <div>
              ...
              {filteredInvalid.map(entry => (
                <div key={entry.server_story.id} className={`flex items-center gap-3 p-3 rounded-xl mt-2 ${isDark ? 'bg-red-950/20 border border-red-800/30' : 'bg-red-50 border border-red-200'}`}>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
          ...
          {filteredNoUpdate.length > 0 && (
            <div>
              ...
              {filteredNoUpdate.map(entry => (
                <div key={entry.server_story.id} className={`flex items-center gap-3 p-3 rounded-xl mt-2 ${isDark ? 'bg-slate-900/20 border border-slate-800/40 opacity-60' : 'bg-gray-100 border border-gray-200 opacity-60'}`}>
                  <div className="flex-1 min-w-0">
                  ...
                  <span className={`px-2 py-1 text-xs rounded-lg ${isDark ? 'text-slate-500 bg-slate-800/40' : 'text-gray-500 bg-gray-200'}`}>Up-to-date</span>
                </div>
              ))}
            </div>
          )}
...
export function StorySyncTabs({
  ...
  return (
    <section className={`rounded-2xl overflow-hidden flex flex-col ${isDark ? 'bg-slate-900/60 border border-slate-800/60' : 'bg-white border border-gray-200'}`}>
      {/* Tab bar with title and settings */}
      <div className={`px-4 pt-4 pb-0 flex items-center justify-between gap-4 ${isDark ? 'border-b border-slate-800/60' : 'border-b border-gray-200'}`}>
