...
import { type CheckUpdatedIntroResponse, type IntroUpdateEntry } from '../../../api';
...
interface CheckUpdatedIntroTabProps {
  data: CheckUpdatedIntroResponse | null;
  loading: boolean;
  error: string;
...
export function CheckUpdatedIntroTab({
  data,
  loading,
  error,
  uploadResults,
...
}: Readonly<CheckUpdatedIntroTabProps>) {
  const isDark = themeMode === 'dark';
  const [search, setSearch] = useState('');
  const [filterSection, setFilterSection] = useState<'all' | 'updated' | 'no_intro' | 'never_updated'>('all');
  const [bulkUploading, setBulkUploading] = useState(false);
  const searchInputRef = useRef<HTMLInputElement>(null);

  const query = search.toLowerCase().trim();
  const filter = (entries: IntroUpdateEntry[]) =>
    entries.filter(
      (entry) =>
        !query ||
  ...
  const filteredNoIntro = filteredEntries.filter((entry) => entry.status === 'no_intro1_file');
  ...
  const handleUpdateAll = async () => {
    ...
    try {
      await Promise.allSettled(
        availableUpdateEntries.map((entry) => onUploadIntro(entry.folder_id, entry.story_id!)),
      );
    } finally {
  ...
  return (
    <div className="flex h-full min-h-[400px] flex-col">
      ...
      <div className="flex-1 p-4">
        {!loading && !data && (
          <EmptyState isDark={isDark} message="Click 'Check Updated Intro' to view all intro update history records." icon={<Icon icon={appIcons.folder} className="h-8 w-8" style={{ color: tertiaryText }} />} />
        )}
        ...
        {data && filterSection === 'all' && filteredEntries.length > 0 && (
          <div className="space-y-2">
            {filteredEntries.map((entry) => (
              <HistoryIntroEntryCard key={entry.folder_id} entry={entry} result={uploadResults.get(entry.folder_id)} isUploading={uploadingIds.has(entry.folder_id)} onUpload={onUploadIntro} isDark={isDark} introFilename={introFilename} />
            ))}
          </div>
        )}
        ...
        {data && filterSection === 'updated' && filteredUpdated.length === 0 && <EmptySection isDark={isDark} message="No updated intro records found." />}
        ...
        {data && filterSection === 'never_updated' && filteredNeverUpdated.length === 0 && <EmptySection isDark={isDark} message="No never-updated intro records found." />}

        {data && filterSection === 'no_intro' && filteredNoIntro.length > 0 && (
          <div className="space-y-2">
            {filteredNoIntro.map((entry) => (
              <HistoryIntroEntryCard key={entry.folder_id} entry={entry} result={uploadResults.get(entry.folder_id)} isUploading={uploadingIds.has(entry.folder_id)} onUpload={onUploadIntro} isDark={isDark} introFilename={introFilename} />
            ))}
          </div>
        )}
        {data && filterSection === 'no_intro' && filteredNoIntro.length === 0 && <EmptySection isDark={isDark} message={`No no-${introFilename} records found.`} />}
      </div>
    </div>
  );
}
...
function HistoryIntroEntryCard({
  entry,
  result,
  isUploading,
  onUpload,
  ...
  const borderColor = isNoIntro ? (isDark ? 'rgba(239,68,68,0.2)' : 'rgba(239,68,68,0.25)') : panelBorder;
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <HistoryIntroStatusChip status={entry.status} isDark={isDark} introFilename={introFilename} />
...
function HistoryIntroStatusChip({ status, isDark, introFilename = 'intro1' }: Readonly<{ status: string; isDark: boolean; introFilename?: string }>) {
  const noIntroLabel = `NO ${introFilename.toUpperCase()}`;
