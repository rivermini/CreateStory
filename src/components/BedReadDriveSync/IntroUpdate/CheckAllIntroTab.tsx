...
import { type CheckAllIntroResponse, type IntroUpdateEntry } from '../../../api';
...
interface CheckAllIntroTabProps {
  data: CheckAllIntroResponse | null;
  loading: boolean;
  error: string;
...
export function CheckAllIntroTab({
  data,
  loading,
  error,
  uploadResults,
    ...
    <div className="flex h-full min-h-[400px] flex-col">
      ...
      <div className="flex-1 p-4">
        {!loading && !data && (
          <EmptyState
            isDark={isDark}
            message="Click 'Check Intro Update' to scan DONE_/EXTENDED_ folders and check intro status."
        ...
        {loading && (
          <div className="flex h-full w-full flex-col items-center justify-center py-16">
            ...
            <p className="text-sm" style={{ color: secondaryText }}>
              Scanning DONE_/EXTENDED_ folders for intro images...
            </p>
          </div>
        )}
        ...
        {data && filterSection === 'all' && (
          <>
            ...
            {filteredNoIntro.length > 0 && (
              <div className="mb-4">
                <SectionHeader label={`No ${introFilename} File (${filteredNoIntro.length})`} color="#f87171" icon={<Icon icon={appIcons.close} className="h-4 w-4" style={{ color: '#f87171' }} />} />
                <div className="space-y-2">
        ...
        {data && filterSection === 'no_intro' && filteredNoIntro.length === 0 && <EmptySection isDark={isDark} message={`No folders missing ${introFilename}.`} />}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <IntroStatusChip status={entry.status} isDark={isDark} introFilename={introFilename} />
...
function IntroStatusChip({ status, isDark, introFilename = 'intro1' }: { readonly status: string; readonly isDark: boolean; readonly introFilename?: string }) {
