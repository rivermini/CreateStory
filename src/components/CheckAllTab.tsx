...
import { type CheckAllResponse, type CoverUpdateEntry } from '../api/client';
...
export function CheckAllTab({
  ...
  const filteredNoCover = filter(data?.no_cover1_file ?? []);
    ...
    <div className="flex flex-col min-h-[400px] h-full">
      ...
      {data && !loading && (
        <div className={`mx-4 mt-3 flex flex-wrap items-center gap-3 px-4 py-2 rounded-xl text-xs ${isDark ? 'bg-black/10' : 'bg-black/5'}`}>
          <div className={`flex items-center gap-1.5 ${isDark ? 'text-white/75' : 'text-black/45'}`}>
            ...
            {(data?.can_update.length ?? 0) + (data?.updated.length ?? 0) + (data?.no_cover1_file.length ?? 0) + (data?.no_server_match.length ?? 0)} DONE_/EXTENDED_ folders
          </div>
          {canUpdateCount > 0 && (
          ...
          {updatedCount > 0 && (
            <div className={`flex items-center gap-1.5 ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>
              <Icon icon={appIcons.check} className="w-3.5 h-3.5" />
      ...
      <div className="p-4 flex-1">
        {!loading && !data && (
          <EmptyState
            isDark={isDark}
            message="Click 'Check Cover Update' to scan DONE_/EXTENDED_ folders and check cover status."
        ...
        {loading && (
          <div className="flex flex-col items-center justify-center py-16 w-full h-full">
            ...
            <p className={`text-sm ${isDark ? 'text-white/65' : 'text-black/45'}`}>Scanning DONE_/EXTENDED_ folders for cover images...</p>
          </div>
        )}
        ...
        {data && filterSection === 'all' && (
          <>
            {filteredCanUpdate.length > 0 && (
              <div className="mb-4">
                <SectionHeader label={`Can Update (${filteredCanUpdate.length})`} color="#34d399" icon={<Icon icon={appIcons.check} className="w-4 h-4 text-emerald-400" />} />
            ...
            {filteredUpdated.length > 0 && (
              <div className="mb-4">
                <SectionHeader label={`Updated (${filteredUpdated.length})`} color="#fbbf24" icon={<Icon icon={appIcons.check} className="w-4 h-4 text-amber-400" />} />
                <div className="space-y-2">
            ...
            {filteredNoCover.length > 0 && (
              <div className="mb-4">
                <SectionHeader label={`No Cover1 File (${filteredNoCover.length})`} color="#f87171" icon={<Icon icon={appIcons.close} className="w-4 h-4 text-red-400" />} />
                <div className="space-y-2">
            ...
            {filteredNoMatch.length > 0 && (
              <div>
                <SectionHeader label={`No Server Match (${filteredNoMatch.length})`} color="#818cf8" icon={<Icon icon={appIcons.folder} className="w-4 h-4 text-indigo-400" />} />
                <div className="space-y-2">
        ...
        {data && filterSection === 'can_update' && filteredCanUpdate.length === 0 && (
          <EmptySection isDark={isDark} message="No folders can be updated." />
        )}
        ...
        {data && filterSection === 'no_cover' && filteredNoCover.length === 0 && (
          <EmptySection isDark={isDark} message="No folders missing cover1.jpg." />
        )}
        ...
        {data && filterSection === 'no_match' && filteredNoMatch.length === 0 && (
          <EmptySection isDark={isDark} message="No folders missing a server story match." />
        )}
      </div>
    </div>
  );
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h4 className={`text-sm font-medium truncate ${isDark ? 'text-white/85' : 'text-black/85'}`}>{entry.story_title || entry.folder_name}</h4>
          </div>
          ...
          {entry.last_updated && (
            <p className={`text-xs mt-1 ${isDark ? 'text-white/40' : 'text-black/25'}`}>last updated: {formatLastUpdated(entry.last_updated)}</p>
          )}
          {result && (
        ...
        <div className="flex flex-col items-end gap-1">
          {canUpload ? (
            ...
            <span className="lg-chip lg-chip-amber">
              <Icon icon={appIcons.check} className="w-3.5 h-3.5" />
              Updated
            </span>
          ) : isNoCover ? (
...
function StatusChip({ status, isDark }: { status: string; isDark: boolean }) {
  const variants: Record<string, { bg: string; text: string; label: string }> = {
    ...
    updated: {
      bg: isDark ? 'rgba(251,191,36,0.15)' : 'rgba(245,158,11,0.08)',
    ...
    no_server_match: {
      bg: isDark ? 'rgba(99,102,241,0.15)' : 'rgba(99,102,241,0.06)',
      text: isDark ? '#818cf8' : '#6366f1',
      label: 'NO MATCH',
    },
    error: {
      bg: isDark ? 'rgba(239,68,68,0.15)' : 'rgba(239,68,68,0.06)',
