...
class ReadNovelMtlCookieUpdateRequest(BaseModel):
    cookies: str = Field(..., min_length=1, description="ReadNovelMtl cookies as Selenium JSON or a raw Cookie header (must include cf_clearance).")
    user_agent: str | None = Field(default=None, description="The exact browser User-Agent that generated cf_clearance.")


class ReadNovelMtlCookieUpdateResponse(BaseModel):
    updated: bool
    cookie_count: int
    has_cf_clearance: bool


class ReadNovelMtlCookieStatusRequest(BaseModel):
    story_url: str | None = Field(default=None, description="Optional ReadNovelMtl story/chapter URL to test against.")

    @field_validator("story_url")
    @classmethod
    def _validate_story_url(cls, value: str | None) -> str | None:
        return validate_external_url(value, ("readnovelmtl.com",), field_name="story_url")


class ReadNovelMtlCookieStatusResponse(BaseModel):
    valid: bool | None
    reason: str
    message: str
    cookie_count: int
...
class ReadNovelMtlBatchStartRequest(BaseModel):
    batch_name: str | None = Field(default=None, max_length=160)
    genres: list[str] | None = Field(default=None, description="ReadNovelMtl discovery source slugs. Empty means all supported sources.")
    max_pages_per_genre: int = Field(default=3, ge=1, le=1000)
    discover_concurrency: int = Field(default=4, ge=1, le=6)
    crawl_concurrency: int = Field(default=6, ge=1, le=8)
    request_delay_seconds: float = Field(default=0.1, ge=0.02, le=15)
...
class ReadNovelMtlBatchCrawlRequest(BaseModel):
    crawl_concurrency: int = Field(default=6, ge=1, le=8)
    request_delay_seconds: float = Field(default=0.1, ge=0.02, le=15)
    max_stories: int | None = Field(default=None, ge=1, le=10000)
    # Auto-run chaining: crawl the queue in fixed-size chunks with a cooldown between them so
    # ReadNovelMtl's per-IP throttle resets each chunk (fully automatic multi-run crawling).
    auto_continue: bool = Field(default=False)
    stories_per_run: int | None = Field(default=None, ge=1, le=10000)
    auto_target_stories: int | None = Field(default=None, ge=0, le=1000000)
    cooldown_seconds: float | None = Field(default=None, ge=0, le=3600)
...
@router.post("/readnovelmtl-cookies", response_model=ReadNovelMtlCookieUpdateResponse)
async def update_readnovelmtl_cookies(request: ReadNovelMtlCookieUpdateRequest, http_request: Request) -> ReadNovelMtlCookieUpdateResponse:
    """Update the saved ReadNovelMtl session cookies (cf_clearance + matching User-Agent)."""
    require_operator_identity(http_request)
    from api.services.readnovelmtl_cookie_service import update_readnovelmtl_cookies as save_cookies
    ...
    logger.info(
        "Updated ReadNovelMtl cookies: %d cookie(s), cf_clearance=%s",
        result["cookie_count"],
        result["has_cf_clearance"],
    )
    return ReadNovelMtlCookieUpdateResponse(**result)


@router.post("/readnovelmtl-cookies/status", response_model=ReadNovelMtlCookieStatusResponse)
async def check_readnovelmtl_cookies(request: ReadNovelMtlCookieStatusRequest, http_request: Request) -> ReadNovelMtlCookieStatusResponse:
    """Check whether saved ReadNovelMtl cookies clear the Cloudflare challenge."""
    require_operator_identity(http_request)
    from api.services.readnovelmtl_cookie_service import check_readnovelmtl_cookies as run_check

    result = run_check(request.story_url)
    logger.info(
        "Checked ReadNovelMtl cookies: valid=%s reason=%s tested_url=%s",
        result["valid"],
        result["reason"],
        result.get("tested_url"),
    )
    return ReadNovelMtlCookieStatusResponse(**result)
...
def _require_readnovelmtl_batch_owner(batch_id: str, request: Request):
...
@router.post("/readnovelmtl-batch/start")
async def start_readnovelmtl_batch(
    request: ReadNovelMtlBatchStartRequest,
    http_request: Request,
) -> dict:
    """Start a one-click ReadNovelMtl batch for latest/ranking-listed stories."""
    require_operator_identity(http_request)
    from api.services.readnovelmtl_batch_service import get_readnovelmtl_batch_service
...
@router.post("/readnovelmtl-batch/{batch_id}/crawl")
async def crawl_readnovelmtl_batch(
    batch_id: str,
    request: ReadNovelMtlBatchCrawlRequest,
    http_request: Request,
) -> dict:
    """Start or resume crawling a discovered ReadNovelMtl batch queue."""
    require_operator_identity(http_request)
    service = _require_readnovelmtl_batch_owner(batch_id, http_request)
    try:
        state = service.start_crawl(
            batch_id=batch_id,
            crawl_concurrency=request.crawl_concurrency,
...
@router.post("/readnovelmtl-batch/{batch_id}/pause")
async def pause_readnovelmtl_batch(batch_id: str, http_request: Request) -> dict:
...
@router.post("/readnovelmtl-batch/{batch_id}/genre-order")
async def reorder_readnovelmtl_batch_genres(batch_id: str, payload: dict, http_request: Request) -> dict:
...
@router.post("/readnovelmtl-batch/{batch_id}/retry-failed")
async def retry_failed_readnovelmtl_batch_rows(batch_id: str, payload: dict, http_request: Request) -> dict:
...
async def list_readnovelmtl_batches(request: Request) -> list[dict]:
    """Return ReadNovelMtl batch history for the current user."""
    from api.services.readnovelmtl_batch_service import get_readnovelmtl_batch_service
...
@router.get("/readnovelmtl-batch/catalog/export")
async def export_readnovelmtl_discovered_catalog(http_request: Request) -> dict:
    """Export all discovered ReadNovelMtl story metadata for backup/restore."""
    require_operator_identity(http_request)
    from api.services.readnovelmtl_batch_service import get_readnovelmtl_batch_service

    return get_readnovelmtl_batch_service().export_discovered_catalog()


@router.get("/readnovelmtl-batch/{batch_id}/catalog/export")
async def export_readnovelmtl_batch_catalog(batch_id: str, request: Request) -> dict:
    """Export discovered ReadNovelMtl story metadata from one selected batch."""
    service = _require_readnovelmtl_batch_owner(batch_id, request)
    try:
        return service.export_batch_catalog(batch_id)
    except KeyError as exc:
        raise HTTPException(status_code=404, detail=str(exc)) from exc
...
@router.post("/readnovelmtl-batch/catalog/import")
async def import_readnovelmtl_discovered_catalog(payload: dict, http_request: Request) -> dict:
    """Import and merge a discovered ReadNovelMtl story catalog backup."""
    require_operator_identity(http_request)
    from api.services.readnovelmtl_batch_service import get_readnovelmtl_batch_service

    try:
        return get_readnovelmtl_batch_service().import_discovered_catalog(
            payload,
            created_by_user_id=current_owner(http_request),
        )
    except ValueError as exc:
...
async def get_readnovelmtl_batch_status(batch_id: str, request: Request) -> dict:
    """Return current ReadNovelMtl batch status."""
    service = _require_readnovelmtl_batch_owner(batch_id, request)
    try:
        return service.get_status(batch_id)
    except KeyError as exc:
...
async def list_readnovelmtl_batch_rows(
    ...
    """Return a lazy page of ReadNovelMtl batch rows."""
    service = _require_readnovelmtl_batch_owner(batch_id, request)
    try:
        return service.list_rows(batch_id, offset=offset, limit=limit, status_filter=status)
    except KeyError as exc:
...
async def get_readnovelmtl_batch_logs(batch_id: str, request: Request) -> dict:
...
@router.delete("/readnovelmtl-batch/{batch_id}")
async def delete_readnovelmtl_batch(batch_id: str, http_request: Request) -> dict:
    """Delete a completed ReadNovelMtl batch history entry and generated output files."""
    require_operator_identity(http_request)
    service = _require_readnovelmtl_batch_owner(batch_id, http_request)
    try:
        service.delete_batch(batch_id)
