"""Development-only cleanup for NovelCrawler-owned runtime data."""
...
OWNED_RUNTIME_MODELS = (
    CrawlOutputFileRecord,
    CrawlSessionRecord,
)

_PROJECT_ROOT = Path(__file__).parent.parent.resolve()
RUNTIME_DIRECTORIES = (_PROJECT_ROOT / "output" / "crawl",)


def _clear_directory(root: Path, containment_root: Path) -> list[str]:
    """Delete children of a service-owned directory without following escapes."""
    root = root.resolve()
...
def clear_owned_runtime_data(
    ...
    """Clear crawl results while preserving crawler cookies and credentials."""
