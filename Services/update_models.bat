...
if defined KOKORO_MODELS_DIR (
    ...
    set "MODELS_DIR=%SCRIPT_DIR%BedReadVoices\api\models"
)
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%SCRIPT_DIR%BedReadVoices\scripts\download-models.ps1" -OutDir "%MODELS_DIR%" -Force
if errorlevel 1 goto fail

echo.
echo [3/4] Checking downloaded model sizes...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  "$m = Get-Item -LiteralPath '%MODELS_DIR%\kokoro-v1.0.onnx'; $v = Get-Item -LiteralPath '%MODELS_DIR%\voices-v1.0.bin'; Write-Host ('kokoro-v1.0.onnx: {0:N0} bytes' -f $m.Length); Write-Host ('voices-v1.0.bin:  {0:N0} bytes' -f $v.Length); if ($m.Length -lt 310MB -or $v.Length -lt 24MB) { throw 'Model files are still too small.' }"
if errorlevel 1 goto fail
